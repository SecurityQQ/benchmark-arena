...
Rules:
- The problem.md / submission.md standard: PROBLEM.md at the repo root is the organizers' authoritative statement of the task, rules, tracks, metric directions, baselines and how/where to submit. Each entry folder holds a SUBMISSION.md describing one solution and the value it achieved per track. Prefer these files over the README when they disagree. Such a repo is a competition. Do not invent rows for SUBMISSION.md files: they are merged in afterwards from the files themselves.
